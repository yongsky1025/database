...
-- 함수
-- 18. FIRST_NAME이 Curtis 인 사람의 first_name, last_name, email, phone_number, job_id 조회
...
-- 27. 부서명, 위치ID, 각 부서별 사원 총수, 부서별 평균 연봉 조회, 평균연봉은 소수점 2자리까지만
-- EMPLOYEES, DEPARTMENTS
SELECT d.DEPARTMENT_NAME, d.LOCATION_ID, count(*) AS "부서별 인원", TRUNC(AVG(e.SALARY),2) AS "부서별 평균 연봉"
FROM EMPLOYEES e
JOIN DEPARTMENTS d ON
	e.DEPARTMENT_ID = d.DEPARTMENT_ID
...
-- 28. Executive 부서에 근부하는 모든 사원들의 LAST_NAME, 부서번호, 부서명, JOB_ID 조회
-- EMPLOYEES, DEPARTMENTS
...
SELECT DISTINCT e1.EMPLOYEE_ID,
	e2.FIRST_NAME || ' ' ||e2.LAST_NAME AS 상대이름
FROM EMPLOYEES e1
JOIN EMPLOYEES e2 ON
...
SELECT E.LAST_NAME, E.DEPARTMENT_ID , E.SALARY
FROM EMPLOYEES e
WHERE (e.department_id, E.SALARY) IN (SELECT e2.department_id, E2.SALARY FROM EMPLOYEES e2 WHERE e.COMMISSION_PCT IS NOT NULL)
...
WHERE e3.DEPARTMENT_ID IN (SELECT DISTINCT e.DEPARTMENT_ID
							FROM EMPLOYEES e
							WHERE e.salary > (SELECT ROUND(AVG(e2.SALARY)) FROM EMPLOYEES e2)
							AND E.LAST_NAME LIKE '%u%');

SELECT e3.EMPLOYEE_ID, e3.LAST_NAME, e3.SALARY
FROM employees e3, (SELECT DISTINCT e.DEPARTMENT_ID
					FROM EMPLOYEES e
					WHERE e.salary > (SELECT ROUND(AVG(e2.SALARY)) FROM EMPLOYEES e2)
...
SELECT e2.EMPLOYEE_ID, e2.first_name || ' ' || e2.last_name AS name, e2.DEPARTMENT_ID, e2.SALARY 
FROM EMPLOYEES e2 
WHERE 	(e2.SALARY, NVL(e2.COMMISSION_PCT,0)) IN (SELECT e.SALARY, NVL(e.COMMISSION_PCT,0)
												FROM EMPLOYEES e
												JOIN DEPARTMENTS d ON
													e.DEPARTMENT_ID = d.DEPARTMENT_ID
												WHERE d.LOCATION_ID = 1700);
